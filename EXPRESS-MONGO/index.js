...
// ============================================
    ...
    app.get('/', async function(req,res) {
        ...
        let dataInExpress = await db.collection( process.env.COLLECTION ).find().toArray();
        console.log(dataInExpress);

        // pass the data to hbs
        res.render('index.hbs', {
            'dataInHbs' : dataInExpress
        });
    ...
    app.post('/create', function (req,res){
        ...
        // organise the data in object format
        let newData = {
            username : username
        }

        // save to MONGODB
        db.collection( process.env.COLLECTION ).insertOne( newData )
    ...
    // .get route 
    app.get('/:taskId/update', async function(req,res){

        // retrieve the taskId from the params object
        let taskId = req.params.taskId;

        // Find in MONGODB using the taskId, using the findOne()
        let dataInExpress = await db.collection( process.env.COLLECTION ).findOne(
            {
                '_id' : ObjectId( taskId )
            }
        );
    ...
    // .post route
    app.post('/:taskId/update', async function(req,res){
    ...
    // .get route
    app.get('/:taskId/delete', async function(req,res){
    ...
    // .post route
    app.post('/:taskId/delete', async function(req,res){
